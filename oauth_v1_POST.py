...
class BurpExtender(IBurpExtender, IHttpListener):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        self.callbacks.setExtensionName("Jimmy POST OAuth Extension v1.1")
    ...
    def processHttpMessage(self, tool_flag, message_is_request, request_response):
        if message_is_request:
            try:
                ...
                headers[0] = "POST %s HTTP/1.1" % final_path_and_query

                # Extract the existing POST body from the original request
                body_offset = request_info.getBodyOffset()
                request_bytes = request_response.getRequest()
                existing_post_body = ''.join(chr(x) for x in request_bytes[body_offset:])  # Extract the body as a string

                # Rebuild the request with updated headers and the existing POST body
                updated_request = self.helpers.buildHttpMessage(headers, existing_post_body.encode('utf-8'))
    ...
    def create_signature_base_string(self, base_url, query_dict, oauth_params):
        ...
        signature_base_string = "POST&" + self.percent_encode(base_url) + "&" + self.percent_encode(query_string)
